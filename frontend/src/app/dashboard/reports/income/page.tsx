...
import type { IncomeReportData, ReportPeriod } from '@/types/report';
...
export default function IncomeReportPage() {
  ...
  const handleExport = async (format: 'csv' | 'pdf') => {
    ...
    try {
      ...
      a.href = url;
      a.download = `income-report-${period}.csv`;
      a.click();
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#17211c]">Income Report</h1>
          <p className="mt-1 text-sm text-slate-500">Income breakdown by category and trend.</p>
        </div>
        ...
        <>
          <div className="mb-6 grid grid-cols-3 gap-4">
            <div className="rounded-[1.5rem] bg-white p-4 shadow-[0_18px_60px_rgba(15,23,42,0.06)]">
              ...
              <p className="mt-1 text-lg font-bold text-emerald-700">{fmt(data.total, data.currency)}</p>
            </div>
            <div className="rounded-[1.5rem] bg-white p-4 shadow-[0_18px_60px_rgba(15,23,42,0.06)]">
            ...
            <div className="rounded-[1.5rem] bg-white p-4 shadow-[0_18px_60px_rgba(15,23,42,0.06)]">
              ...
              <p className="mt-1 text-lg font-bold text-[#17211c]">{fmt(data.averageIncome, data.currency)}</p>
            </div>
          </div>

          {data.categories.length > 0 && (
            <div className="mb-6 rounded-[1.5rem] bg-white p-5 shadow-[0_18px_60px_rgba(15,23,42,0.06)]">
              <h2 className="mb-3 font-semibold text-[#17211c]">Top Income Categories</h2>
          ...
          {data.transactions.length > 0 && (
            <div className="rounded-[1.5rem] bg-white shadow-[0_18px_60px_rgba(15,23,42,0.06)]">
              <div className="border-b px-4 py-3">
                <h2 className="font-semibold text-[#17211c]">Income Transactions</h2>
              </div>
                ...
                <tbody>
                  {data.transactions.map((tx) => (
                    <tr key={tx._id} className="border-b hover:bg-[#f4f6f3]">
                      ...
                      <td className="px-4 py-3 text-right font-medium text-emerald-700">{fmt(tx.amount, data.currency)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {data.transactions.length === 0 && (
            <div className="rounded-[1.5rem] bg-white p-12 text-center shadow-[0_18px_60px_rgba(15,23,42,0.06)]">
              <p className="text-slate-500">No confirmed income transactions for this period.</p>
            </div>
          )}
        </>
      )}
